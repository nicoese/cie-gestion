...
public class AlumnoService {
    ...
    @Transactional
    public AlumnoApp updateAlumno(AlumnoApp alumno, String alumnoId) {

        AlumnoApp antiguoAlumno = alumnoRepository
                .findById(alumnoId)
                .orElseThrow(
                        () -> new IllegalStateException("el alumno no existe")
                );
        ...
        modulos.stream()
                ...
                (modulo -> {
                    Nota nota = notaService.createNota(
                            new Nota(modulo, user, curso, PENDIENTE, 0.0, null, null));
